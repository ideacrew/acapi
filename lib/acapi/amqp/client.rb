  ...
  module Amqp
    class Client
      ...
      def publish_processing_failed(delivery_info, properties, payload, err)
        ...
        notify(
          @processing_failed_queue,
          error_properties(delivery_info, properties).merge({:body => JSON.dump(error_message)})
        )
      ...
      def error_properties(delivery_info, properties, exception = nil)
        new_properties = properties.to_hash.dup
        new_headers = new_properties.delete(:headers) || {}
        ...
        new_headers.merge(new_properties)
      end
